...
const cv::Size MAIN_WINDOW_SIZE(450, 420);
const cv::Size LRF_WINDOW_SIZE(400, 420);
	...
	double scale)
{
	...
	const cv::Point SHIFT(margin, MAIN_WINDOW_SIZE.height - ridge - margin);
	/* 四角 */
	cv::rectangle(img,
		cv::Point(0, 0) + SHIFT,
...
void draw_robot(cv::Mat img, Chassis& chassis, Imu& imu, size_t margin, double scale)
{
	...
	const cv::Point SHIFT(MAIN_WINDOW_SIZE.width - ridge / 2 - margin,
		MAIN_WINDOW_SIZE.height - ridge / 2 - margin);
	...
	jibiki::ShareVar<int>& pushed_key)
{
	try
	{
		...
		/* Mainの画像を作成 */
		cv::Mat main_img = cv::Mat::zeros(MAIN_WINDOW_SIZE, CV_8UC3);
		/* LRFの画像を作成 */
		cv::Mat lrf_img1 = cv::Mat(PROCESS_IMG_SIZE, CV_8UC3);
		cv::Mat lrf_img2 = cv::Mat(LRF_WINDOW_SIZE, CV_8UC3);

		/* Main*/
		cv::namedWindow("Main Window");        /* ウィンドウを作成 */
		cv::moveWindow("Main Window", 415, 35);/* ウィンドウを移動 */
		/*LRF*/
		cv::namedWindow("LRF window");        /* ウィンドウを作成 */
		cv::moveWindow("LRF window", 0, 0);   /* ウィンドウを移動 */
		...
		while (jibiki::thread::manage(exit_flag))
		{
			...
			/*-----------------Main Window-----------------*/
			/* 描画データの用意 */
			draw_current_method(main_img, current_method, 1);
			draw_chassis(main_img, g_chassis, 3);
			draw_controller(main_img, g_controller, 5);
			draw_executing_order(main_img, executing_order, 7);
			draw_analog_stick(main_img, g_controller, 40, 0.5);
			draw_robot(main_img, g_chassis, g_imu, 40, 0.5);

			/* 描画 */
			cv::imshow("Main Window", main_img);                  /* ウィンドウに描画 */
			main_img = cv::Mat::zeros(MAIN_WINDOW_SIZE, CV_8UC3); /* 画像をクリア */
			pushed_key = cv::waitKey(1);                /* キー入力 */

			/*-----------------LRF Window-----------------*/
			/* 描画データの用意 */
			g_LRF.get_drawdata(lrf_img1, lrf_img2, LRF_WINDOW_SIZE);

			/* 描画 */
			cv::imshow("LRF window", lrf_img2);
			lrf_img1 = cv::Mat(PROCESS_IMG_SIZE, CV_8UC3, cv::Scalar(0, 0, 0));
		}
	}
