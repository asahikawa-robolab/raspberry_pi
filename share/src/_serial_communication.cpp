...
namespace jibiki
{
	...
	*
	 ...
	 *
	-----------------------------------------------*/
	ProcParamCom::ProcParamCom(std::string json_path,
							   ShareVal<bool> &exit_flag,
							   std::vector<jibiki::ProcParamCom::ComFunc> com_func)
	{
		try
		{
			/* スレッドの実行の管理 */
			if (!enable_thread("com"))
				return;
			...
			for (size_t i = 0; i < root_array.size(); ++i)
			{
				...
				name.push_back(com_array[0].get<std::string>());
				enable.push_back(com_array[1].get<bool>());
				path.push_back(com_array[2].get<std::string>());
			}
			...
			{
				usleep(50);
				for (size_t i = 0; i < com_func.size(); ++i)
					if (enable[i])
						com_func[i](path[i], name[i]);
			}
		}
